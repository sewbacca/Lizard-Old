...
static Move* gen_pawn_captures(const Position& pos, Color side, Move *list) {
	...
	auto append = [&](bitboard moves, int offX, Move* add(Square from, Square to, Color side, Move* list, const Position&)) -> void {
		while(moves) {
...
static Move create (Square from, Square to, Color side, Piece capture, Piece promotion, bool enpassant = false, bool doublepush = false) {
	Move res;
...
static Move* add_pawn_en_passant(Square from, Square to, Color side, Move* list, const Position& pos) {
	bool isProm = side == WHITE ? rank(to) == 7 : rank(to) == 0;
