...
static void next_move(Move* list, Move* end, Position& pos) {
	...
	// };
	Move* maxmove = std::max_element(list, end, [&pos](const Move& largest, const Move& cmp) {
...
static void index(Move* list, Move* end, Position& pos) {
	...
	for(Move* move = list; move < end; move++) {
		...
		if(move->capture() != NO_PIECE) {
			if(move->isEnPassant())
				move->score += 1;
			move->score = capture + VALUES[piece_type(move->capture())] * 100 - VALUES[piece_type(move->piece())];
		}
...
void search(SearchInfo* info_ptr, Position* pos_ptr) {
	...
	*/
	...
	Move* end = gen_pseudo(pos, pos.side, moves);
	index(moves, end, pos);
	std::sort(moves, end, [](const Move& before, const Move& after) {
