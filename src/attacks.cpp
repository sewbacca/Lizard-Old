...
bitboard attacks_p(Square sq, Color col) { return PAWN_ATKS[col][sq]; }

bitboard defenders_p(Square sq, Color col) { return PAWN_ATKS[swap(col)][sq]; }

bitboard attacks_n(Square sq) { return KNIGHT_ATKS[sq]; }

bitboard attacks_k(Square sq) { return KING_ATKS[sq]; }
...
bitboard attacks_q(Square sq, bitboard occupied) { return attacks_b(sq, occupied) | attacks_r(sq, occupied); }
...
bool is_in_check(const Position& pos, Color side)
{
	...
	{
		if (is_attacked(pos.piecepos[king][i], swap(side), pos)) return true;
	}
...
size_t init_attacks()
{
	...
	return Magic::init_magic() + sizeof(KNIGHT_ATKS) + sizeof(KING_ATKS) + sizeof(PAWN_ATKS);
}